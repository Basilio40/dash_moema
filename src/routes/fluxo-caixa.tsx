      ...
      <Panel title="Entradas × Saídas por mês">
        <ResponsiveContainer width="100%" height={360}>
          <ComposedChart data={fc} stackOffset="sign">
            ...
            <Bar dataKey="entradasEfet" name="Entradas efetivadas" stackId="a" fill="#10B981" />
            <Bar
              dataKey="entradasPrev"
                ...
                {fc.map((m) => (
                  <tr key={m.mes} className="border-b border-border/50">
                    ...
                    <td className="text-right text-[color:var(--success)]">
                      {brl(m.entradasEfet + m.entradasPrev)}
                    </td>
                    <td className="text-right text-destructive">
                      {brl(m.saidasEfet + m.saidasPrev)}
                    </td>
                    <td
                      className={`text-right font-semibold ${
                        m.liquido >= 0 ? "text-[color:var(--success)]" : "text-destructive"
                      }`}
                    >
                      {brl(m.liquido)}
                    </td>
                    <td
                      className={`text-right font-bold ${
                        m.saldoAcum >= 0 ? "text-foreground" : "text-destructive"
                      }`}
                    >
                      {brl(m.saldoAcum)}
                    </td>
